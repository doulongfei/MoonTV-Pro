...
const EpisodeSelector: React.FC<EpisodeSelectorProps> = ({
  ...
  const handleRefreshSources = useCallback(async () => {
    ...
    // 筛选出检测失败的源
    const failedSources = availableSources.filter((source) => {
      const sourceKey = `${source.source}-${source.id}`;
    ...
    // 清除失败源的检测记录
    const newAttemptedSources = new Set(attemptedSources);
    const newVideoInfoMap = new Map(videoInfoMap);

    failedSources.forEach((source) => {
      const sourceKey = `${source.source}-${source.id}`;
      newAttemptedSources.delete(sourceKey);
      newVideoInfoMap.delete(sourceKey);
      attemptedSourcesRef.current.delete(sourceKey);
    });

    setAttemptedSources(newAttemptedSources);
    setVideoInfoMap(newVideoInfoMap);

    // 重新检测失败的源
    const batchSize = Math.ceil(failedSources.length / 2);

    for (let start = 0; start < failedSources.length; start += batchSize) {
      const batch = failedSources.slice(start, start + batchSize);
      ...
      {activeTab === 'sources' && (
        <div className='flex flex-col h-full mt-4'>
          ...
          {!sourceSearchLoading &&
            ...
            availableSources.length > 0 &&
            (() => {
              const hasFailedSources = availableSources.some((source) => {
                const sourceKey = `${source.source}-${source.id}`;
                const videoInfo = videoInfoMap.get(sourceKey);
                return videoInfo && videoInfo.hasError;
              });
                ...
                hasFailedSources && (
                  <div className='flex-shrink-0 mb-3 px-2'>
                    <button
                      ...
                      className='w-full flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-green-500 hover:bg-green-600 disabled:bg-gray-400 disabled:cursor-not-allowed text-white font-medium transition-colors duration-200'
                    >
                      {isRefreshing ? (
                        <>
                          <svg
